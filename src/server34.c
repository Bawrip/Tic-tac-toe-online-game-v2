...
#define PORT "7784"   // порт, который мы слушаем
#define MAX_GAME 30
...
void sendNext(int id, int code, int data, char *data2) {
	...
	while(1) {
		if((mySend(id, code, data, data2, servSock)) == -1) {
			if ((num = connectToNextServer()) == -1) {
				break;
			} else {
				servSock = num;
			}
...
void sendClient(int id, int code, int data, char *data2, int sockClient) {
	if (mySend(id, code, data, data2, sockClient) == -1) {
		...
		fprintf(fileLog, "time Client id:%d not responding", j); 
		fflush(fileLog);
	}
}
int connectToNextServer()
...
int main(void)
{
     ...
     * 
     * 
     * изменить имя лога для каждого сервера
     * 
     * 
     * */
    fileLog = fopen("logServ4", "a");
    
    fd_set read_fds;  // временный сет дескрипторов для select()

    int listener, recvServSock = -1;     // дескриптор слушающего сокета
            ...
            if (FD_ISSET(i, &read_fds)) { // есть!
                if (i == listener) {
					...
					if ((nbytes = myRecv(i)) <= 0) {
                        ...
                        if (nbytes == 0) {
                            // соединение закрыто
                            fprintf(fileLog, "time Server %s:%s disconnected", rootServers->address, rootServers->port); 
							fflush(fileLog);
                        } else {
                            fprintf(fileLog, "time Server %s:%s error", rootServers->address, rootServers->port); 
							fflush(fileLog);
                        }
                        close(i); // bye!
						...
						if (rcvCode == WAIT)
						{
							...
							myRecv(i);
							if (myFullRecvMessage.data == games[myFullRecvMessage.id]->op1)
							{
					...
					if ((nbytes = myRecv(i)) <= 0) {
                        ...
                        if (nbytes == 0) {
                            // соединение закрыто
                            fprintf(fileLog, "time Client id:%d disconnected", j); 
							fflush(fileLog);
                        } else {
                            fprintf(fileLog, "time Client id:%d error", j); 
							fflush(fileLog);
                        }
                        close(i); // bye!
                        FD_CLR(i, &master); // удаляем из мастер-сета
						...
						if (rcvCode == WAIT)
						{
							...
							if (ptrlist == NULL)
							{
								...
								games[gameCount]->id2 = myFullRecvMessage.id;
								
								clients[clientsCount] = i;
								sendNext(ptrlist->id, MAKE_CHOICE, myFullRecvMessage.id, "");
								
								sendClient(gameCount, NORM, 0, "", games[gameCount]->op1);
								sendClient(gameCount, NORM, 0, "", games[gameCount]->op2);
