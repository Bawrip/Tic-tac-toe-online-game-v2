...
#define PORT "7783"   // порт, который мы слушаем
#define MAX_GAME 30
...
void sendClient(int id, int code, int data, char *data2, int sockClient) {
	if (mySend(id, code, data, data2, sockClient) == -1) {
		...
		fprintf(fileLog, "Client socket %d id:%d not responding\n", sockClient, j); 
		fflush(fileLog);
...
{   
    /*
     * изменить имя лога в соответствии с именем сервера
     * */
    fileLog = fopen("logServ2", "a");
                ...
                if (i == listener) {
                    ...
                    if (newfd == -1) {
                        ...
                        printf("selectserver: new connection from %s on "
                            ...
                            newfd);
                        fprintf(fileLog, "Client sock : %d connected\n", newfd); 
						fflush(fileLog);
                    }
